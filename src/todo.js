...
function updateTodo(oldTodo, todoEdit) {
  ...
  for (let [key, value] of Object.entries(todoEdit)) {
    if (todoEdit[key] !== "" && oldTodo[key] !== value) {
      //check if same value not present in old todo
      if (key == "deadline") {
...
export function getTodos(array, uiFilter) {
  ...
  if (uiFilter === "all") {
    ...
    todoArray = array.filter((todo) =>
      datefns.isWithinInterval(todo.deadline, {
        start: hoy,
